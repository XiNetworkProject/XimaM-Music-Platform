...
export async function POST(req: NextRequest) {
  try {
    ...
    const checkout = await stripe.checkout.sessions.create({
      ...
      success_url: `${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/studio?checkout=success&session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/studio?checkout=cancel`,
      metadata: {
