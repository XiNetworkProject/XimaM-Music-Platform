...
export function GET(req: NextRequest) {
  // Cette route est utilisée pour initialiser le WebSocket
  return NextResponse.json({ message: 'WebSocket endpoint' });
}

export function POST(req: NextRequest) {
  // Cette route peut être utilisée pour des communications HTTP avec le WebSocket
  return NextResponse.json({ message: 'WebSocket communication' });
}
...
export function initializeSocketIO(server: NetServer) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('disconnect', () => {
      ...
      for (const [userId, socketId] of Array.from(activeConnections)) {
