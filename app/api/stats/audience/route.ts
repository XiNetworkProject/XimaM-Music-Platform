...
export async function GET(request: NextRequest) {
  try {
    ...
    if (trackIds.length === 0) {
      return NextResponse.json({ countries: {}, devices: {}, os: {}, browsers: {} });
    }
    ...
    for (const v of views || []) {
      const country = v.country?.trim() ? v.country.toUpperCase() : 'Inconnu';
...
function normDev(d: string) { const s = d.toLowerCase(); if (/android|iphone|mobile/.test(s)) return 'Mobile'; if (/tablet|ipad/.test(s)) return 'Tablette'; return 'Desktop'; }
function devFromUA(ua?: string|null) { if (!ua) return 'Desktop'; const s = ua.toLowerCase(); if (/mobile|iphone|android/.test(s)) return 'Mobile'; if (/ipad|tablet/.test(s)) return 'Tablette'; return 'Desktop'; }
function osFromUA(ua?: string|null) { if (!ua) return 'Autre'; const s = ua.toLowerCase(); if (s.includes('windows')) return 'Windows'; if (s.includes('mac os')||s.includes('macintosh')) return 'macOS'; if (s.includes('android')) return 'Android'; if (/iphone|ipad|ios/.test(s)) return 'iOS'; if (s.includes('linux')) return 'Linux'; return 'Autre'; }
function brFromUA(ua?: string|null) { if (!ua) return 'Autre'; const s = ua.toLowerCase(); if (s.includes('edg/')) return 'Edge'; if (s.includes('chrome/')) return 'Chrome'; if (s.includes('safari/')&&!s.includes('chrome/')) return 'Safari'; if (s.includes('firefox/')) return 'Firefox'; return 'Autre'; }
function toPct(map: Map<string,number>) { const entries = Array.from(map.entries()); const total = entries.reduce((s,[,v])=>s+v,0)||1; const obj: Record<string,number>={}; for (const [k,v] of entries) obj[k]=Math.round((v/total)*1000)/10; return obj; }
