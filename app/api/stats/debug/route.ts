...
export async function GET(_req: NextRequest) {
  ...
  try {
    ...
    try {
      const { data, error } = await dbAdmin.from('track_views').select('*').limit(1);
    ...
    try {
      const { data: userTracks } = await dbAdmin
    ...
    try {
      ...
      const { data: aiRows } = await dbAdmin
      ...
      if (ids.length > 0) {
        ...
        const { data: sampleEvents } = await dbAdmin
