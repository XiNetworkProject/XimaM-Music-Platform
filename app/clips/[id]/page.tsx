    ...
    <main className="v2-creation chambre-public-clip min-h-screen bg-[var(--v2-bg)] px-5 py-8 text-[var(--v2-text)] sm:px-10 sm:py-12">
      <div className="v2-clip-stage">
        ...
        <section className="max-w-xl">
          ...
          <Link href={`/profile/${encodeURIComponent(clip.creator.username)}`} className="mt-4 inline-flex font-medium text-[var(--v2-accent)]">
            @{clip.creator.username || creator}
          </Link>
          <div className="v2-clip-source">
            <p className="v2-kicker">SON ORIGINAL</p>
          ...
          <p className="mt-6 text-sm text-[var(--v2-muted)]">{clip.likesCount} j’aime · {clip.commentsCount} commentaires</p>
        </section>
      </div>
    </main>
  );
