...
test('Discovery presents approved Chambre identity and retains every real-content section', async () => {
  ...
  assert.match(source, /Suivez<br \/><em>l’onde\.<\/em>/);
...
test('420 UI handlers and 212 protected calls match pre-redesign AST, allowing only the exact loader cleanup fix', async () => {
  ...
  for (const path of musicSources) {
    const text = (await read(path)).replaceAll('\r\n', '\n');
    const ast = ts.createSourceFile(path, text, ts.ScriptTarget.Latest, true, ts.ScriptKind.TSX);
    const events = [];
    const calls = [];
    function visit(node) {
