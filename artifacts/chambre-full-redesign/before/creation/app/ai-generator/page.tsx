...
/** Messages d'erreur Suno en franÃ§ais (doc: 400, 401, 404, 405, 413, 429, 430, 455, 500) */
...
// Composant "Orb" de statut pour la gÃ©nÃ©ration
...
/* â”€â”€ Portal-based model dropdown (matches library ContextMenu style) â”€â”€ */
...
function AIGeneratorContent() {
  ...
  /** Cooldown aprÃ¨s 430 (rate limit): dÃ©sactive le bouton GÃ©nÃ©rer pendant quelques secondes */
  ...
  // Ã‰tats pour la bibliothÃ¨que des gÃ©nÃ©rations
  ...
  // Ã‰tats pour le panneau de track sÃ©lectionnÃ©e
  ...
  const [studioRemixPermissions, setStudioRemixPermissions] = useState<RemixPermissionsValue>(DEFAULT_REMIX_PERMISSIONS);
  ...
  // Onglet mobile : CrÃ©er | BibliothÃ¨que
  ...
  const normalizePanelWidths = useCallback((leftCandidate: number, rightCandidate: number) => {
    ...
    // Conserve la prÃ©fÃ©rence: droite plus compacte que gauche sans bloquer les drags.
    if (right > left) {
      right = left;
    }
  ...
  // Charger la bibliothÃ¨que (mÃªme logique que ai-library)
  ...
  // Filtrer les gÃ©nÃ©rations (mÃªme logique que ai-library)
  ...
  // Convertir les tracks individuelles en gÃ©nÃ©rations groupÃ©es par generation_id
  const generationsFromTracks = React.useMemo(() => {
    ...
    allTracks.forEach((track: any) => {
      // L'API retourne peut-Ãªtre generation_id directement ou via generation.id
      ...
      // CrÃ©er une gÃ©nÃ©ration virtuelle si elle n'existe pas
      if (!generationMap.has(genId)) {
        // Utiliser les infos de gÃ©nÃ©ration si disponibles (via JOIN), sinon utiliser la track
    ...
    // CrÃ©er les gÃ©nÃ©rations complÃ¨tes avec leurs tracks
  ...
  // Fusion des deux sources (API generations + generationsFromTracks) pour que A/B et le reste voient toutes les musiques gÃ©nÃ©rÃ©es
  const recentGenerationsSorted = React.useMemo(() => {
    ...
    generations.forEach((g) => byId.set(String(g.id), g)); // lâ€™API Ã©crase si doublon (mÃ©tadonnÃ©es plus complÃ¨tes)
  ...
  const resolveTrackMedia = (track: AITrack | any) => {
    ...
    return {
      // En bibliothÃ¨que, prÃ©fÃ©rer l'URL audio finale; garder le stream en fallback.
  ...
  const resolveLiveTrackMedia = (track: any) => {
    ...
    // En live, le stream est gÃ©nÃ©ralement disponible avant l'URL audio finale.
  ...
  // Jouer une track IA (mÃªme logique que ai-library)
  ...
  const playAITrack = async (track: AITrack, generation: AIGeneration) => {
    ...
    try {
      ...
      // Fallback robuste: si le chargement Ã©choue, tenter une re-hydratation + retry 1 fois.
      const refreshed = await hydrateTrackFromSuno(track, generation);
      if (!refreshed) {
  ...
  const playGenerationQueue = async (generation: AIGeneration) => {
    ...
    // Queue + lecture (plus â€œbibliothÃ¨queâ€ et plus pratique pour rÃ©Ã©couter)
    setQueueAndPlay(playable as any, 0);
    notify.music('Lecture', `Lecture de ${playable.length} piste(s)`);
  };

  // Fonction pour jouer une gÃ©nÃ©ration
  ...
  // Toggle favori (mÃªme logique que ai-library)
  ...
  // TÃ©lÃ©charger une track (mÃªme logique que ai-library)
  ...
  // Charger la bibliothÃ¨que au montage (mÃªme logique que ai-library)
  useEffect(() => {
    loadLibrary();
  ...
  useEffect(() => {
    ...
    // VÃ©rifier si retour de Checkout Stripe
    try {
      const params = new URLSearchParams(window.location.search);
      const sid = params.get('session_id');
      if (sid) {
        // Petit dÃ©lai pour laisser le webhook agir
        setTimeout(async () => {
          ...
          // Si pas d'effet, tente une vÃ©rification cÃ´tÃ© serveur (secours)
          try {
  ...
  useEffect(() => {
    ...
    const sourceTrack = params.get('sourceTrackId') || params.get('sourceTrack') || params.get('track') || '';
    const sourceTrackType = (params.get('sourceTrackType') === 'ai_track' ? 'ai_track' : 'track') as 'track' | 'ai_track';
    ...
    const applySource = (source: any) => {
      const titleLabel = source?.title || sourceTitle || 'ce morceau';
      const prefill = source?.prefill || {};
      const tags = Array.isArray(prefill.tags) ? prefill.tags.filter(Boolean).map(String) : [];
      const genre = Array.isArray(prefill.genre) ? prefill.genre.filter(Boolean).map(String) : [];
      const styleParts = [...genre, prefill.mood, ...tags].filter(Boolean).map(String);
      const styleLabel = styleParts.join(', ') || sourceStyle || 'variation IA Synaura';
      const inspirationLine = `Inspiration: ${styleLabel}. Do not copy melodies, lyrics, audio, stems or private files. Create an original Synaura track.`;

      selectGenerationMode(modeParam === 'remix' ? 'remix' : 'simple');
      setRemixUploadUrl(null);
      setRemixSourceDurationSec(undefined);
      if (sourceTrack) setRemixSourceTrackId(sourceTrack);
      setRemixSourceLabel(titleLabel);
      setTitle((current) => current.trim() ? current : `Variation de ${titleLabel}`.slice(0, 80));
      setStyle((current) => current.trim() ? current : `${styleLabel}. ${inspirationLine}`);
      setDescription((current) => current.trim() ? current : (prefill.prompt || prefill.description || `Créer une variation IA originale inspirée par "${titleLabel}". ${inspirationLine}`));
      if (tags.length) setSelectedTags((current) => current.length ? current : tags.slice(0, 8));
    ...
    const hydrateSource = async () => {
      if (!sourceTrack) {
        applySource({ title: sourceTitle, prefill: { genre: sourceStyle ? [sourceStyle] : [] } });
        return;
      }
      try {
        const params = new URLSearchParams({ sourceTrackId: sourceTrack, sourceTrackType });
        const response = await fetch(`/api/remixes/source?${params.toString()}`, { cache: 'no-store' });
        const json = await response.json().catch(() => ({}));
        if (!response.ok) throw new Error(json?.error || 'source unavailable');
        if (!cancelled) applySource(json.source);
      } catch (error: any) {
        if (!cancelled) {
          setSourceContext({
            mode: 'remix',
            id: sourceTrack,
            sourceTrackType,
            title: sourceTitle || 'ce morceau',
            style: sourceStyle,
            audioAttached: false,
            warning: error?.message || 'Variation IA non autorisée pour ce morceau.',
          });
        }
      }
    };
  ...
  // CatÃ©gories de tags faÃ§on Suno (scroll vertical) â€” fusion avec les suggestions API
  ...
  // MÃ©moire locale des prÃ©fÃ©rences (dev simple)
  ...
  const handleTagClick = (tag: string) => {
    ...
    if (!isActive) {
      ...
      if (customMode) {
        ...
        // En mode simple, injecter dans la description si non prÃ©sent
        setDescription(prev => {
      ...
      // Remplacer la suggestion sÃ©lectionnÃ©e par une nouvelle
      ...
      // RÃ©insÃ©rer Ã  la fin de la liste correspondante pour reproposer plus tard
      if (styleSuggestions.indexOf(tag) === -1 && vibeSuggestions.indexOf(tag) === -1) {
        // RÃ©injecter cÃ´tÃ© styles par dÃ©faut
  ...
  const openTrackPanel = (track: GeneratedTrack) => {
    console.log('ðŸŽµ Ouverture du panneau pour la track:', track);
  ...
  // Source de vÃ©ritÃ© unique pour le statut de gÃ©nÃ©ration
  React.useEffect(() => {
  ...
  const generateAutoLyrics = useCallback(async () => {
    ...
    const seedText = [
      ...
      .filter(Boolean)
      .join(' | ');
    // Doc Suno Generate Lyrics: prompt max 200 caractÃ¨res
    ...
    try {
      ...
      // Si encore pending, on poll les dÃ©tails quelques secondes supplÃ©mentaires.
  ...
  /** Contexte de la piste actuellement en lecture (queue) pour charger waveform/paroles synchronisÃ©es */
  ...
  /** Contexte effectif : piste en lecture si dispo (taskId+audioId), sinon piste sÃ©lectionnÃ©e â†’ waveform/lyrics liÃ©s Ã  la musique jouÃ©e */
  ...
  /** La waveform/paroles affichÃ©es correspondent Ã  la piste en lecture â†’ on affiche la progression, sinon 0 */
  ...
  const toggleGenerationVisibility = useCallback(async () => {
    ...
    try {
      if (track?.id) {
        ...
        setGenerations((prev) =>
          prev.map((g) => ({
            ...
            tracks: (g.tracks || []).map((t: any) =>
              String(t.id) === String(track.id) ? { ...t, is_public: effectivePublic } : t
            ),
          }))
        );
        ...
        setGenerations((prev) =>
          prev.map((g) => (String(g.id) === String(generation.id) ? { ...g, is_public: effectivePublic } : g))
        );
        setSelectedGeneration((prev) =>
          prev && String(prev.id) === String(generation.id) ? { ...prev, is_public: effectivePublic } : prev
        );
      }
  ...
  }, [publishingVisibility, pushLog, selectedTrackForVisibility, selectedGenerationForVisibility, selectedVisibilityState, studioRemixPermissions]);
  ...
  // Doc Suno: stream_audio_url dispo en 30â€“40s, audio_url (final) en 2â€“3 min. On stocke lâ€™URL finale en prioritÃ© une fois dispo pour tÃ©lÃ©chargement/liste correcte.
  React.useEffect(() => {
  ...
  // DÃ¨s qu'une gÃ©nÃ©ration passe en completed, synchroniser la bibliothÃ¨que
  React.useEffect(() => {
  ...
  // Une fois la bibliothÃ¨que rafraÃ®chie aprÃ¨s "completed", remplacer generatedTracks par la version API (URLs finales garanties)
  React.useEffect(() => {
  ...
  const generateMusic = async () => {
    ...
    try {
      ...
      // Synchroniser le modÃ¨le effectif et informer en cas de downgrade
      if (data?.model) {
      ...
      if (data.taskId) {
        ...
        console.log('ðŸŽµ GÃ©nÃ©ration Suno initiÃ©e en arriÃ¨re-plan:', data.taskId);
  ...
  // Mise Ã  jour du compte Ã  rebours chaque seconde pendant le cooldown (force re-render pour le libellÃ©)
  useEffect(() => {
  ...
  // Recharge les droits de creation de la piste selectionnee (par defaut : remix desactive).
  useEffect(() => {
    const t = studioInspectorTrack as any;
    if (!t) {
      setStudioRemixPermissions(DEFAULT_REMIX_PERMISSIONS);
      return;
    }
    const remixVisibility = ['everyone', 'followers', 'disabled'].includes(t.remix_visibility) ? t.remix_visibility : 'disabled';
    setStudioRemixPermissions({
  ...
  if (!session) {
    return (
      <SynauraAppShell contentClassName="v2-creation">
        <SynauraTopBar
        ...
        <div className="flex min-h-[70vh] items-center justify-center">
          <div className="v2-panel max-w-xl px-8 py-10 text-center">
            <Music className="mx-auto mb-6 h-10 w-10 text-[var(--v2-accent)]" />
            <p className="v2-kicker">AI Generator</p>
            <h1 className="my-4 text-3xl font-normal">Une idée mérite d’être entendue.</h1>
  ...
  return (
    <SynauraAppShell contentClassName="v2-creation v2-ai-workspace flex flex-col overflow-hidden !pb-0 sm:!pb-0">
      <SynauraTopBar
        searchLabel="Rechercher une inspiration, un son ou un createur..."
        secondaryHref="/upload"
        secondaryLabel="Upload"
      ...
      <div className="v2-ai-workspace-body">
        ...
        <section className="v2-ai-toolbar">
          <div className="v2-ai-identity">
            <h1>AI Generator</h1>
            <p>{studioStateLabel} <span className="hidden xl:inline">· {studioModeCopy}</span></p>
          </div>
        ...
        >
          ...
          >
            <div className="v2-ai-composer-heading">
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <p className="v2-kicker">01 · Intention</p>
                  <h2 className="mt-2">À quoi ressemble ton idée ?</h2>
                </div>
                <button
                  type="button"
                  onClick={() => setMobileCreateOpen(false)}
                ...
                {([
                  ...
                  return (
                    <button
                      ...
                      className={`rounded-[1rem] border px-2 py-3 text-xs font-semibold transition ${
                        active
                          ? 'border-[var(--v2-line)] bg-[var(--v2-surface)] text-white shadow-[0_10px_24px_rgba(20,15,10,0.16)]'
                          : 'border-[var(--v2-line)] bg-[var(--v2-raised)] text-[#6e5f54] hover:bg-[var(--v2-raised)]'
                      }`}
                    >
              ...
              {isRemixMode ? (
                <div className="mt-3 space-y-3">
                  ...
                  <label className="block">
                    <span className="mb-2 block text-[11px] font-semibold uppercase tracking-[0.16em] text-[#8b7868]">Variation souhaitee</span>
                    <textarea
                      value={description}
                      onChange={(event) => setDescription(event.target.value)}
                      disabled={isGenerationDisabled}
                      placeholder="Ex: version plus intime, tempo plus rapide, garder les paroles mais changer la production..."
                      className="min-h-[92px] w-full resize-none rounded-[1.1rem] border border-[var(--v2-line)] bg-[var(--v2-raised)] px-4 py-3 text-sm font-semibold leading-6 text-[var(--v2-text)] outline-none placeholder:text-[#9b8d82] focus:border-[var(--v2-line)]"
                    />
                  </label>
                </div>
              ) : null}
            ...
            <div className="space-y-4 p-4">
              {generationModeKind === 'simple' ? (
                <label className="block">
                  <span className="mb-2 flex items-center justify-between gap-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-[#8b7868]">
                    Prompt principal
                    <button
                      type="button"
                  ...
                  <textarea
                    ...
                    className="min-h-[170px] w-full resize-none rounded-[1.2rem] border border-[var(--v2-line)] bg-[var(--v2-raised)] px-4 py-3 text-sm font-semibold leading-6 text-[var(--v2-text)] outline-none placeholder:text-[#9b8d82] focus:border-[var(--v2-line)]"
                  />
                </label>
              ) : (
                <div className="space-y-3">
                  <label className="block">
                    ...
                    <input
                      ...
                      className="h-12 w-full rounded-full border border-[var(--v2-line)] bg-[var(--v2-raised)] px-4 text-sm font-semibold text-[var(--v2-text)] outline-none placeholder:text-[#9b8d82] focus:border-[var(--v2-line)]"
                    />
                  </label>
                  <label className="block">
                    <span className="mb-2 flex items-center justify-between gap-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-[#8b7868]">
                    ...
                    <textarea
                      ...
                      className="min-h-[125px] w-full resize-none rounded-[1.2rem] border border-[var(--v2-line)] bg-[var(--v2-raised)] px-4 py-3 text-sm font-semibold leading-6 text-[var(--v2-text)] outline-none placeholder:text-[#9b8d82] focus:border-[var(--v2-line)]"
                    />
                  </label>
                  <label className="block">
                    <span className="mb-2 flex items-center justify-between gap-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-[#8b7868]">
                    ...
                    <textarea
                      ...
                      className="min-h-[150px] w-full resize-none rounded-[1.2rem] border border-[var(--v2-line)] bg-[var(--v2-raised)] px-4 py-3 text-sm font-semibold leading-6 text-[var(--v2-text)] outline-none placeholder:text-[#9b8d82] focus:border-[var(--v2-line)] disabled:bg-[var(--v2-raised)]"
                    />
                  </label>
                </div>
              )}
                        ...
                        return (
                          <button
                            ...
                            className={`min-w-0 rounded-xl border px-3 py-2 text-left text-xs font-semibold transition ${
                              picked ? 'border-[var(--v2-line)] bg-[var(--v2-raised)] text-[var(--v2-accent)]' : 'border-[var(--v2-line)] bg-[var(--v2-raised)] text-[#5f5650] hover:bg-[var(--v2-raised)]'
                            }`}
                          >
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  className={`rounded-[1rem] border px-3 py-3 text-left text-xs font-semibold transition ${
                    isInstrumental ? 'border-[var(--v2-line)] bg-[var(--v2-surface)] text-white' : 'border-[var(--v2-line)] bg-[var(--v2-raised)] text-[#5f5650]'
                  }`}
                >
                ...
                <button
                  ...
                  className="rounded-[1rem] border border-[var(--v2-line)] bg-[var(--v2-raised)] px-3 py-3 text-left text-xs font-semibold text-[#5f5650] transition hover:bg-[var(--v2-surface)] disabled:opacity-45"
                >
              ...
              <div className="rounded-[1.25rem] border border-[var(--v2-line)] bg-[var(--v2-raised)] p-3">
                <div className="mb-3 flex items-center justify-between">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#8b7868]">Couleurs sonores</p>
                    ...
                    return (
                      <button
                        ...
                        className={`rounded-full border px-3 py-1.5 text-[11px] font-semibold transition ${
                          active ? 'border-[var(--v2-line)] bg-[var(--v2-surface)] text-white' : 'border-[var(--v2-line)] bg-[#f7efe4] text-[#6e5f54] hover:bg-[#fff7ec]'
                        }`}
                      >
                        {tag}
                      </button>
              ...
              <div className="rounded-[1.25rem] border border-[var(--v2-line)] bg-[var(--v2-surface)] p-3 text-white shadow-[0_18px_46px_rgba(20,15,10,0.16)]">
                <div className="mb-3 flex items-center justify-between gap-3">
                  <div>
                    <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--v2-muted)]">Demarrer vite</p>
                    <p className="mt-0.5 text-[11px] font-semibold text-[var(--v2-muted)]">{aiStudioPresets.length} intentions pretes pour Suno</p>
                  </div>
                      ...
                      >
                        <span className="flex min-w-0 items-start gap-2.5">
                          ...
                          <span className="min-w-0 flex-1">
                            ...
                            <span className={`mt-0.5 block line-clamp-2 text-[10px] font-semibold leading-4 ${active ? 'text-[#6e5f54]' : 'text-[var(--v2-muted)]'}`}>
                              {preset.description}
                            </span>
                          </span>
                          {active ? (
              ...
              <div className="pt-2">
                <p className="px-1 text-[10px] font-semibold uppercase tracking-[0.18em] text-[var(--v2-accent)]">2 &middot; Reglages</p>
              </div>

              <div className="rounded-[1.25rem] border border-[var(--v2-line)] bg-[var(--v2-raised)] p-3">
                <div className="mb-3 flex items-center justify-between">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#8b7868]">Modele &amp; duree</p>
                  <div className="flex items-center gap-2">
                  ...
                  {(['V4_5', 'V4_5PLUS', 'V5', 'V5_5'] as const).map((model) => (
                    <button
                      ...
                      className={`rounded-full px-3 py-2 text-[11px] font-semibold transition ${
                        modelVersion === model ? 'bg-[var(--v2-surface)] text-white' : 'bg-[var(--v2-surface)] text-[#6e5f54] hover:bg-[#efe0ce]'
                      }`}
                    >
                ...
                <div className="mt-3 grid grid-cols-3 gap-2">
                  {([60, 120, 180] as const).map((duration) => (
                    <button
                      ...
                      className={`rounded-full px-3 py-2 text-[11px] font-semibold transition ${
                        generationDuration === duration ? 'bg-[var(--v2-accent)] text-white' : 'bg-[var(--v2-surface)] text-[#6e5f54] hover:bg-[#efe0ce]'
                      }`}
                    >
                      {duration}s
                    </button>
                ...
                {customMode && (
                  <div className="mt-3">
                    <button
                      ...
                      className="flex w-full items-center justify-between rounded-full bg-[var(--v2-surface)] px-3 py-2 text-[11px] font-semibold text-[#6e5f54] transition hover:bg-[#efe0ce]"
                    >
                      Reglages avances
                      ...
                      <div className="mt-3 space-y-3">
                        {([
                          ...
                          <label key={label} className="block">
                            <span className="mb-1 flex items-center justify-between text-[11px] font-semibold text-[#6e5f54]">
                              {label}
                              <span>{value}%</span>
                            </span>
                            <input
                        ...
                        <label className="block">
                          <span className="mb-1 block text-[11px] font-semibold text-[#6e5f54]">Tags a eviter</span>
                          <input
                            value={negativeTags}
              ...
              <div className="pt-2">
                <p className="px-1 text-[10px] font-semibold uppercase tracking-[0.18em] text-[var(--v2-accent)]">3 &middot; Publication</p>
              </div>

              <div className="rounded-[1.25rem] border border-[var(--v2-line)] bg-[var(--v2-accent)] p-3">
                ...
                <div className="mt-3 flex items-center justify-between gap-2 rounded-full bg-[var(--v2-raised)] px-3 py-2">
                  <span className="text-[11px] font-semibold text-[#5b4a80]">Cout de cette creation</span>
                ...
                <button
                  ...
                  className="mt-3 inline-flex h-12 w-full items-center justify-center gap-2 rounded-full bg-[var(--v2-accent)] px-5 text-sm font-semibold text-white shadow-[0_14px_34px_rgba(115,87,198,0.28)] transition hover:scale-[1.01] hover:bg-[var(--v2-accent)] disabled:cursor-not-allowed disabled:opacity-45"
                >
          ...
          <main className="h-full min-h-0 min-w-0 overflow-hidden">
            <section className="v2-ai-results h-full min-h-0 overflow-hidden">
              <LibraryMiddlePanel
                tracks={allTracks}
          ...
          <aside className="v2-ai-inspector hidden min-w-0 space-y-4 lg:block lg:h-full lg:overflow-y-auto">
            <section className="rounded-[1.5rem] border border-[var(--v2-line)] bg-[var(--v2-surface)] p-4 text-white shadow-[0_20px_70px_rgba(20,15,10,0.18)]">
              ...
              <div className="mt-4 overflow-hidden rounded-[1.25rem] border border-white/10 bg-[var(--v2-raised)]">
                ...
                <div className="p-3">
                  <h3 className="truncate text-lg font-semibold">{studioInspectorTrack?.title || 'Piste non selectionnee'}</h3>
            ...
            <section className="rounded-[1.5rem] border border-[var(--v2-line)] bg-[var(--v2-surface)] p-4 shadow-[0_20px_70px_rgba(20,15,10,0.08)]">
              <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#8b7868]">Session</p>
            ...
            <section className="rounded-[1.5rem] border border-[var(--v2-line)] bg-[var(--v2-surface)] p-4 shadow-[0_20px_70px_rgba(20,15,10,0.08)]">
              <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#8b7868]">Memoire</p>
              ...
              <div className="mt-3 max-h-56 space-y-2 overflow-y-auto">
                {logs.slice(0, 8).map((line) => (
                  <div key={line.id} className="rounded-[1rem] bg-[var(--v2-raised)] px-3 py-2">
                    <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-[#8b7868]">{line.level} - {line.at}</p>
                    <p className="mt-1 text-xs font-semibold text-[var(--v2-text)]">{line.msg}</p>
                  </div>
                ))}
                {logs.length === 0 && <p className="rounded-[1rem] bg-[var(--v2-raised)] px-3 py-4 text-center text-xs font-semibold text-[#8b7868]">Aucun evenement pour l'instant.</p>}
              </div>
            </section>
          </aside>
          </>
  ...
  return (
    <SynauraAppShell contentClassName="max-w-[1660px]">
      ...
      <div className="relative z-10 mx-auto max-w-[1600px] px-2.5 py-3 pb-[calc(150px+env(safe-area-inset-bottom,0px))] sm:px-4 sm:py-4 lg:pb-4">
        ...
        {/* LAYOUT "Studio Pro" : panneaux fixes (scroll interne uniquement) ; sur mobile onglets GÃ©nÃ©rer / BibliothÃ¨que */}
        <div ref={containerRef} className="grid grid-cols-12 gap-3 lg:flex lg:items-stretch lg:gap-3">
            ...
            <div className="flex-1 overflow-y-auto px-3 space-y-3 pb-4 lg:pb-3 pt-3">
              ...
              <section className="space-y-3 flex flex-col min-h-0">
                          ...
                          <React.Fragment key={cat.id}>
                            {cat.tags.map((tag) => {
                              ...
                              return (
                                ...
                                >
                                  {tag}
                                  {active && <span className="text-[9px] ml-0.5">âœ•</span>}
                                </button>
                              );
                            })}
                          </React.Fragment>
                ...
                <div className="rounded-2xl border border-white/10 bg-[var(--v2-raised)] p-3 space-y-3">
                  ...
                  {/* Pas d'options avancÃ©es en mode simple */}
                </div>
              )}
          ...
          {/* Mini-player flottant mobile â€” positioned above BottomNav (56px + safe-area + gap) */}
            ...
            >
              ...
              >
                ...
                <div className="border-t border-white/[0.04] px-4 py-2.5 flex items-center gap-4 text-[10px] text-[var(--v2-muted)]">
                  <span><kbd className="font-mono bg-[var(--v2-raised)] px-1.5 py-0.5 rounded text-[var(--v2-muted)]">â†‘â†“</kbd> naviguer</span>
