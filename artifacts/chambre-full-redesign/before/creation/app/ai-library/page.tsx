...
export default function AILibrary() {
  ...
  return (
    <SynauraAppShell contentClassName="v2-creation v2-ai-library !max-w-[1560px]">
      ...
      <main className="pb-32">
        <div className="w-full overflow-hidden">
          ...
          <header className="v2-creative-header">
            <div>
              <p className="v2-kicker">Collection de créations</p>
              <h1>Les idées qui ont pris forme.</h1>
              <p>Écoute, retrouve une version, prépare une publication ou repars d’une piste avec AI Generator.</p>
            </div>
            <Link href="/ai-generator" className="v2-create-primary"><Sparkles size={16} />Nouvelle création</Link>
          </header>
          <dl className="v2-library-stats">
          ...
          <div className="text-center py-12">
            ...
            {generations.map((generation, index) => (
              <motion.div
                ...
                className="rounded-lg p-6 border border-[var(--border)] hover:bg-[var(--v2-raised)] transition-colors"
              >
                ...
                <div className="space-y-3 mb-4">
                  {generation.tracks?.map((track) => {
                    ...
                    return (
                    <div key={track.id} className="flex items-center gap-3 p-3 bg-[var(--v2-raised)] rounded-lg">
          ...
          {allTracks.length > 0 && (
            <SynauraPanel className="mt-8 p-3 sm:p-6">
              ...
              {allTracks.map((track) => {
                ...
                return (
                <div key={track.id} className="flex items-center gap-3 py-4 border-b border-[var(--v2-line)]">
                  <div className="w-12 h-12 rounded-lg overflow-hidden bg-[var(--v2-raised)] flex items-center justify-center">
                    <TrackCover src={track.image_url || null} videoSrc={videoMeta.videoUrl} posterSrc={videoMeta.posterUrl || track.image_url || null} title={track.title} className="h-full w-full" rounded="rounded-none" objectFit="cover" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className="font-medium truncate">{track.title}</h4>
                  ...
                  <div className="flex gap-2">
                    ...
                    <Link href={studioLinkForTrack(track, 'style')} className="p-2 bg-[var(--v2-raised)] hover:bg-[var(--v2-raised)] rounded-lg transition-colors">
                      <Sparkles className="w-4 h-4" />
                    </Link>
                    <Link href={studioLinkForTrack(track, 'remix')} className="p-2 bg-[var(--v2-raised)] hover:bg-[var(--v2-raised)] rounded-lg transition-colors">
                      <Repeat2 className="w-4 h-4" />
                    </Link>
                    {canGenerateVideo ? (
                      <button
