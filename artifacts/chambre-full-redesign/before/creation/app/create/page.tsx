...
const CREATIVE_PATHS = {
  idea: {
    ...
    text: 'Quelques mots, une ambiance, des paroles. AI Generator t’aide à explorer une idée musicale ; tu gardes la main sur les versions et leur publication.',
...
function CreateHubContent() {
  ...
  return (
    <SynauraAppShell contentClassName="v2-create v2-creation !max-w-[1680px]">
      <SynauraTopBar searchLabel="Rechercher un son, un profil ou une playlist..." />
      <main className="v2-create-main">
        <header className="v2-create-intro">
          <HandoffReturn fallbackHref="/" fallbackLabel="Retour" />
          <div><p className="v2-kicker">L’atelier Synaura</p><h1 className="v2-heading">Qu’est-ce que tu veux créer ?</h1><p className="v2-intro">Pars de ce que tu as. Une idée, un son, une envie de faire ensemble.</p></div>
        </header>
        ...
        <div className="v2-create-composition">
          <section className="v2-create-intentions" aria-label="Point de départ créatif">
            <p className="v2-kicker">Ton point de départ</p>
            <div className="v2-intent-choices" role="group" aria-label="Choisir une intention">
              {CREATIVE_INTENTS.map(({ id, label, icon: Icon }, index) => (
                <button key={id} type="button" aria-pressed={creativeIntent === id} aria-controls="creative-path" onClick={() => setCreativeIntent(id)} className="v2-intent-choice"><span className="v2-intent-number">0{index + 1}</span><Icon aria-hidden="true" size={19} /><span>{label}</span><ArrowRight aria-hidden="true" size={17} /></button>
              ))}
            </div>
          ...
          <section className="v2-create-path" id="creative-path" aria-labelledby="creative-path-title">
            <p className="v2-kicker">{creativePath.kicker}</p><h2 id="creative-path-title">{creativePath.title}</h2><p className="v2-create-path-description">{creativePath.text}</p>
            <ol className="v2-create-sequence">{creativePath.steps.map((step, index) => <li key={step}><span>0{index + 1}</span>{step}</li>)}</ol>
          ...
          <aside className="v2-create-studio">
            <span className="v2-kicker">Un espace pour aller plus loin</span>
            <div className="v2-create-studio-mark" aria-hidden="true"><span>01</span><span>02</span><span>03</span></div>
