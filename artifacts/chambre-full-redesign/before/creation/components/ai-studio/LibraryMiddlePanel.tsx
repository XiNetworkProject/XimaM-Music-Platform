...
export function LibraryMiddlePanel({
  ...
  return (
    <div className="v2-ai-library-panel flex h-full min-h-0 w-full flex-col bg-[var(--v2-bg)] text-[var(--v2-text)]">
      {/* ── Toolbar ── */}
      <div className="v2-ai-library-toolbar shrink-0 space-y-3 border-b border-[var(--v2-line)] px-4 pb-4 pt-5">
        <div className="flex items-center justify-between gap-2 px-0.5">
          <div className="min-w-0">
            <div className="flex min-w-0 items-center gap-2">
              <h2 className="truncate text-xl font-normal tracking-[-0.02em] text-[var(--v2-text)]">Tes créations</h2>
              {selectedFolder !== 'all' ? (
      ...
      <div ref={listRef} className="min-h-0 flex-1 overflow-y-auto px-3 pb-[calc(8rem+env(safe-area-inset-bottom,0px))] pt-3 lg:pb-3">
        ...
        {loading ? (
          <div className="py-20 text-center">
            <div className="mx-auto mb-4 h-8 w-8 animate-spin rounded-full border-2 border-[var(--v2-line)] border-t-[#171313]" />
            <span className="text-sm font-bold text-[var(--v2-muted)]">Chargement...</span>
          </div>
        ) : error ? (
          <div className="py-20 text-center text-sm font-bold text-red-700">{error}</div>
        ) : filtered.length === 0 ? (
          <div className="py-20 text-center">
            <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-[1.1rem] border border-[var(--v2-line)] bg-[var(--syn-surface)] shadow-[0_12px_30px_rgba(30,25,20,0.08)]">
              <Sparkles className="h-6 w-6 text-[var(--v2-muted)]" />
            </div>
            <p className="text-sm font-semibold text-[var(--v2-text)]">Aucune piste</p>
            <p className="mt-1 text-[11px] font-bold text-[var(--v2-muted)]">Change les filtres ou lance une nouvelle création.</p>
          </div>
        ) : (
          <>
