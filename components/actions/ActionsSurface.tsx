...
function QueueContent() {
  ...
  return <div data-organization-queue className="space-y-4">
    {current ? <div className="rounded-xl bg-[var(--syn-soft)] p-4"><p className="text-xs text-[color-mix(in_srgb,var(--syn-accent)_65%,var(--syn-text-primary))]">Morceau courant</p><p className="mt-1 break-words font-bold">{current.title}</p><p className="text-xs">{current.artist.name}</p></div> : <p>Aucun morceau courant.</p>}
