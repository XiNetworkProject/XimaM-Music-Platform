...
export default function OnboardingFlow() {
  ...
  if (!ready || transitioning) {
    return (
      <main className="v2-personal v2-personal--onboarding v2-onboarding relative grid min-h-[100svh] place-items-center overflow-hidden bg-[var(--syn-background)] px-6 text-center text-[var(--syn-text-primary)]" aria-busy="true">
        ...
        <div className="relative"><SynauraLogo size={104} className="mx-auto drop-shadow-[0_0_24px_rgba(115,87,198,.34)]" priority decorative /><p className="mt-5 text-sm font-black text-[var(--syn-text-secondary)]">{transitioning ? 'Ton univers prend forme…' : 'Synaura écoute tes préférences…'}</p></div>
      </main>
    );
  }

  return (
    <main className="v2-personal v2-personal--onboarding v2-onboarding relative min-h-[100svh] overflow-hidden bg-[var(--syn-background)] px-3 py-[max(env(safe-area-inset-top),0.75rem)] text-[var(--syn-text-primary)] sm:px-6 sm:py-8">
      <div className="chambre-onboarding-material pointer-events-none absolute left-1/2 top-1/2 h-[48rem] w-[48rem] -translate-x-1/2 -translate-y-1/2 rounded-full" aria-hidden />
      <div className="relative mx-auto w-full max-w-6xl">
        <header className="mb-5 flex items-center justify-between gap-4 px-1">
          <span className="flex items-center gap-2 text-sm font-black"><SynauraLogo size={36} decorative /> Synaura</span>
        ...
        <section className="v2-onboarding-panel relative min-h-[min(44rem,calc(100svh-7rem))] border-t border-[var(--syn-border)] py-8 sm:py-12" data-chambre-step={step}>
          ...
          <AnimatePresence mode="wait" initial={false}>
            ...
            {step === 2 ? <motion.div key="tastes" {...motionProps(reduced)} className="v2-onboarding-step"><StepHeading eyebrow="1 · Tes couleurs" title="Quels univers t’attirent&nbsp;?" description="Ces choix alimentent les goûts déjà utilisés par Découvrir et les recommandations." /><div className="mt-7 grid grid-cols-2 gap-2.5 sm:grid-cols-4">{ONBOARDING_UNIVERSES.map((option) => { const Icon = UNIVERSE_ICON[option.id]; const active = universes.includes(option.id); return <button key={option.id} type="button" aria-pressed={active} onClick={() => setUniverses((current) => current.includes(option.id) ? current.filter((id) => id !== option.id) : [...current, option.id])} className={`syn-interactive flex min-h-28 flex-col items-center justify-center gap-2 rounded-[var(--syn-radius-lg)] border p-3 text-center ${active ? 'border-[var(--syn-accent)] bg-[color-mix(in_srgb,var(--syn-accent)_14%,transparent)]' : 'border-[var(--syn-border)] bg-[var(--syn-soft)] hover:border-[var(--syn-accent)]'}`}><span className={`grid h-10 w-10 place-items-center rounded-2xl ${active ? 'bg-[var(--v2-accent-fill)] text-white' : 'bg-[var(--syn-surface)]'}`}><Icon className="h-5 w-5" /></span><span className="text-xs font-black leading-tight">{option.label}</span>{active ? <Check className="h-3.5 w-3.5 text-[var(--syn-accent)]" /> : null}</button>; })}</div><StepActions onBack={isEdit ? undefined : () => setStep(1)} onNext={() => setStep(3)} /></motion.div> : null}

            {step === 3 ? <motion.div key="intentions" {...motionProps(reduced)} className="v2-onboarding-step"><StepHeading eyebrow="2 · Ton mouvement" title="Qu’as-tu envie de faire ici&nbsp;?" description="Ces intentions préparent tes raccourcis. Aucune fonction ne sera cachée." /><div className="mt-7 grid gap-2.5 sm:grid-cols-2">{CREATOR_INTENTIONS.map((option) => { const Icon = INTENTION_ICON[option.id]; const active = intentions.includes(option.id); return <button key={option.id} type="button" aria-pressed={active} onClick={() => setIntentions((current) => current.includes(option.id) ? current.filter((id) => id !== option.id) : [...current, option.id])} className={`syn-interactive flex min-h-16 items-center gap-3 rounded-[var(--syn-radius-lg)] border p-3 text-left ${active ? 'border-[var(--syn-accent)] bg-[color-mix(in_srgb,var(--syn-accent)_14%,transparent)]' : 'border-[var(--syn-border)] bg-[var(--syn-soft)] hover:border-[var(--syn-accent)]'}`}><span className={`grid h-10 w-10 shrink-0 place-items-center rounded-2xl ${active ? 'bg-[var(--v2-accent-fill)] text-white' : 'bg-[var(--syn-surface)]'}`}><Icon className="h-4 w-4" /></span><span className="text-sm font-black">{option.label}</span>{active ? <Check className="ml-auto h-4 w-4 text-[var(--syn-accent)]" /> : null}</button>; })}</div><StepActions onBack={() => setStep(2)} onNext={() => setStep(4)} /></motion.div> : null}

            {step === 4 ? <motion.div key="final" {...motionProps(reduced)} className="flex min-h-[34rem] flex-col items-center justify-center text-center"><div className="relative grid h-32 w-32 place-items-center overflow-visible"><motion.span className="absolute inset-0 rounded-full bg-[radial-gradient(circle_at_36%_28%,rgba(255,255,255,.8),rgba(217,109,99,.7)_15%,rgba(115,87,198,.72)_48%,rgba(74,158,170,.58)_70%,transparent_73%)] shadow-[var(--syn-glow-accent)]" animate={reduced ? undefined : { scale: [0.96,1.04,0.96] }} transition={{ duration: 5, repeat: Infinity }} /><SynauraLogo size={92} className="relative" decorative /></div><p className="mt-6 text-xs font-black uppercase tracking-[0.2em] text-[var(--syn-accent-coral)]">Tes choix sont prêts</p><h1 className="mt-3 text-4xl font-black tracking-[-0.05em] sm:text-5xl">Entre dans Live Synaura.</h1><p className="mt-3 max-w-lg text-sm font-semibold text-[var(--syn-text-secondary)]">{universes.length ? `${universes.length} univers` : 'Aucun univers imposé'} · {intentions.length ? `${intentions.length} envies` : 'tu exploreras librement'}</p><SynauraButton className="mt-8" variant="accent" size="lg" loading={saving} onClick={() => void save(false)}>Faire vibrer mon univers <ArrowRight className="h-4 w-4" /></SynauraButton><button type="button" onClick={() => setStep(3)} className="syn-interactive mt-3 min-h-11 rounded-full px-4 text-xs font-black text-[var(--syn-text-secondary)]"><ArrowLeft className="mr-1 inline h-3.5 w-3.5" /> Modifier mes choix</button></motion.div> : null}
          </AnimatePresence>
        </section>
      </div>
    </main>
