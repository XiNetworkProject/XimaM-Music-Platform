  ...
  if (!ready || transitioning) {
    return (
      <main className="relative grid min-h-[100svh] place-items-center overflow-hidden bg-[var(--syn-background)] px-6 text-center text-[var(--syn-text-primary)]" aria-busy="true">
        ...
        <div className="relative"><SynauraLogo size={104} className="mx-auto drop-shadow-[0_0_24px_rgba(115,87,198,.34)]" priority decorative /><p className="mt-5 text-sm font-black text-[var(--syn-text-secondary)]">{transitioning ? 'Ton Aura prend forme…' : 'Synaura écoute tes préférences…'}</p></div>
      </main>
    );
  }
