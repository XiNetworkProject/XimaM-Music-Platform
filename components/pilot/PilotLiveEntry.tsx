...
export default function PilotLiveEntry({ model }: { model: LivePilotModel }) {
  ...
  return <section className="pilot-entry" aria-labelledby="pilot-entry-title" data-pilot-entry-track={track?._id} data-playing={playing}
    ...
    <div className="pilot-entry-copy"><p className="pilot-kicker">{firstName ? `Bonjour ${firstName} / Votre Live` : 'Live / Votre première rencontre'}</p><h1 id="pilot-entry-title">Laissez le son<br /><em>vous trouver.</em></h1><p className="pilot-entry-intent">Des sons à ressentir. Des gens à rencontrer.<br />Votre prochain coup de cœur est peut-être là.</p><nav className="pilot-entry-shortcuts" aria-label="Explorer et créer">{[{label:'Découvrir',sub:'Sons & artistes',href:'/discover'},{label:'Radar',sub:'Une autre sélection',href:'/radar'},{label:'Studio IA',sub:'Donner forme au son',href:'/ai-generator'},{label:'Événements',sub:'La scène Synaura',href:'/city'}].map(shortcut => <button key={shortcut.href} onClick={() => model.navigateFromEntry(shortcut.href)}><span>{shortcut.label}<small>{shortcut.sub}</small></span><ArrowUpRight size={17}/></button>)}</nav></div>
    <div className="pilot-entry-record">
      {track ? <><LiveArtwork key={track._id} className="pilot-entry-art" active><TrackCover trackId={track._id} src={track.coverUrl} videoSrc={track.coverVideoUrl} posterSrc={track.coverVideoPosterUrl} title={track.title} alt={`Pochette de ${track.title}`} autoPlayVideo playOnHover={false} rounded="rounded-none" className="live-track-cover" /><span aria-hidden="true">{playing ? 'À L’ÉCOUTE / LE VOYAGE CONTINUE' : '01 / LE DÉBUT DU VOYAGE'}</span></LiveArtwork><div className="pilot-entry-track"><div><h2>{track.title}</h2><button className="live-entry-artist" data-context-surface-trigger-key={`entry-profile-${track._id}`} onClick={event => profile(track.artist.username, event.currentTarget)}>{track.artist.name}<ArrowUpRight size={14}/></button></div><button onClick={toggle} aria-label={`${playing ? 'Mettre en pause' : 'Écouter'} ${track.title}`}>{playing ? <Pause /> : <Play />}</button></div><div className="pilot-entry-stats"><span><Headphones size={14}/>{count(track.plays)} écoutes</span><span>{Math.floor(track.duration / 60)}:{String(Math.floor(track.duration % 60)).padStart(2,'0')}</span><span>{track.genre?.slice(0,2).join(' · ')}</span></div><LiveEntrySocial track={track} openTrack={openTrack} /></> : <p className="pilot-entry-empty">Votre fil est prêt à être exploré.</p>}
