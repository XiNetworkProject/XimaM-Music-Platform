...
function coverUrl(track: any): string {
  return String(track?.image || track?.image_url || track?.imageUrl || '/default-cover.svg').trim();
}
...
export default function StudioTimeline({
  ...
  const onPlayTrack = (t: StudioTrack) => {
    ...
    const queue = tracks.map((x) => ({
      ...
      audioUrl: x.audioUrl || '',
      coverUrl: x.coverUrl || '/default-cover.svg',
      genre: ['IA', 'Généré'],
      plays: 0,
      likes: [],
      comments: [],
  ...
  return (
    <div className="experience-studio-timeline h-full min-h-0 flex flex-col gap-3">
              ...
              return (
                <div key={g.taskId} className="rounded-2xl border border-white/10 bg-black/25 p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)]">
                  <div className="flex items-center gap-3">
                    <div className="grid h-10 w-10 place-items-center rounded-2xl bg-[var(--v2-raised)] text-cyan-100">
                      {chip.icon}
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="truncate text-sm font-semibold text-white">{g.title || 'Generation Suno'}</div>
                      <div className="truncate text-[11px] text-[var(--v2-muted)]">{g.style || g.prompt || g.taskId}</div>
                    </div>
                    <div className="text-right">
                      <div className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--v2-muted)]">{chip.label}</div>
                      <div className="text-[11px] text-[var(--v2-muted)]">{Math.round(g.progress || 0)}%</div>
                    </div>
                  </div>

                  <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-[var(--v2-raised)]">
                    <div
                      className="h-full rounded-full bg-[var(--v2-raised)]"
                        ...
                        <div
                          ...
                          className={`group relative overflow-hidden rounded-2xl border p-3 transition ${
                            isReady
                              ...
                              : 'border-white/10 bg-[var(--v2-raised)]'
                          }`}
                        >
                          <div
                          ...
                          <div className="relative flex items-center gap-3">
                            <div className="grid h-12 w-12 shrink-0 place-items-center overflow-hidden rounded-2xl border border-white/10 bg-black/40">
                              {isReady ? (
                                ...
                                <Waves className="h-5 w-5 animate-pulse text-[var(--v2-muted)]" />
                              )}
                            </div>
                            <div className="min-w-0 flex-1">
                              <div className="text-[10px] font-semibold uppercase tracking-[0.2em] text-[var(--v2-muted)]">
                            ...
                            <button
                              ...
                              className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-[var(--v2-raised)] text-[var(--v2-text)] transition hover:scale-105 disabled:scale-100 disabled:bg-[var(--v2-raised)] disabled:text-[var(--v2-muted)]"
      ...
      <details className="experience-queue-disclosure" open={mobileTab === 'timeline'}>
        <summary><span>File de génération</span><small>Exécution et relances</small><Clock size={14} aria-hidden="true" /></summary>
        <QueuePanel />
      </details>

      {mobileTab !== 'library' ? (
        <div className="panel-suno experience-generation-history overflow-hidden">
          <div className="p-3 border-b border-border-secondary">
            <div className="v2-kicker">Le fil de la session</div>
            <div className="text-sm font-semibold text-foreground-primary">Historique des générations</div>
          </div>

          <div className="min-h-0 overflow-y-auto p-3">
            {sortedJobs.length === 0 ? (
              <div className="experience-small-empty"><Waves size={26} aria-hidden="true" /><strong>Le prochain son reste à écrire.</strong><p>Chaque génération laissera ici son intention, son modèle et son état.</p></div>
            ) : (
