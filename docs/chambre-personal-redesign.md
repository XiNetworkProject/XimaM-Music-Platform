...
## Addendum — reprise bornée après retour utilisateur

La livraison précédente ne couvrait pas suffisamment les surfaces réellement ouvertes depuis les pages. Cette reprise ne constitue pas une nouvelle affirmation de « refonte complète » : elle traite uniquement trois écarts précis.

- **Cloche Notifications :** panneau explicitement composé avec les tokens Chambre, en-tête et compteur, filtres qui se replient, liste de lecture, états push/chargement/vide et footer permanent. Le corps entier défile et le footer réserve la safe area. Chaque notification utilise maintenant un bouton natif pour son action existante ; la suppression reste un bouton frère toujours visible. Les actions « Tout marquer comme lu » et « Tout supprimer » ont un nom explicite. Les types et couleurs sémantiques du système de toasts restent conservés ; seule l’icône partagée reçoit une classe de présentation ciblée à l’intérieur du panneau.
- **Bibliothèque :** les trois portails existants (création, actions d’un morceau, édition) disposent de véritables en-têtes, corps scrollables et footers avec safe area. Les champs ont des labels reliés, les fermetures sont nommées, les contrôles public/privé exposent leur état. Le Play de carte est visible en permanence, tactile et clavier compris. Les fonctions de création/édition/file/partage/ordre/couverture restent les mêmes.
- **Messages invité :** garde et bouton de connexion inchangés ; ajout du `SynauraAppShell`, de la marque/du retour du `SynauraTopBar` et de la navigation déjà existants, avec une composition responsive qui présente les fonctionnalités réelles (discussions, amis/demandes, groupes). Aucun faux fil de messages ni faux utilisateur n’est affiché.

Autorisation de cette reprise : wrappers, éléments de présentation, classes, labels et composition uniquement. **Aucun import ajouté, aucun assouplissement de l’empreinte AST originale, aucun handler modifié.** L’action de notification a été déplacée sans changer son expression vers un bouton natif. Les trois fichiers TSX conservent exactement leur empreinte de code non-JSX et d’expressions comportementales face aux copies `artifacts/chambre-resumption/before/personal/` ; Bibliothèque et Messages conservent également l’empreinte de la première reprise.

`tests/chambre-personal-resumption.test.mjs` : **9 tests ciblés PASS**, auxquels s’ajoutent les **16 tests personnels existants PASS** importés sans modification pour réutiliser le comparateur strict (25 au total lors de cette commande). Contrôles : parsing TSX/CSS, scope, empreintes AST, notifications nommées et clavier, trois corps scrollables et safe areas, labels, Play sans hover, navigation invitée, contraste AA des boutons principaux. Aucun résultat de navigateur n’est déduit de ces tests de source.

Restent explicitement non validés par ce sous-lot : revue authentifiée des trois portails et de la cloche avec contenu réel, clavier OS Android/Gboard, NVDA réel et mutations utilisateur. Les modals Boosters identifiés dans l’audit restent **inchangés et à traiter séparément**. Les autres écrans personnels/services ne sont pas déclarés achevés visuellement par ce correctif. Build, navigateur et validation générale restent centralisés ; aucune opération de production n’a été effectuée.
