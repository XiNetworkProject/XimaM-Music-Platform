...
catch (e) { if (e.code !== 'ENOENT') throw e; baseline = currentHashes; await fs.writeFile(baselineFile, JSON.stringify(baseline, null, 2), { flag: 'wx' }); }
...
// Ordered oldest first: a later reprise must never replace an earlier source
// baseline, including when that earliest version happens to match the candidate.
const snapshotRoots = [
  'artifacts/chambre-full-redesign/before',
  'artifacts/chambre-resumption/before',
  'artifacts/chambre-continuation/before',
  'artifacts/chambre-signature/before',
  'artifacts/chambre-experience/before',
];
const seenSnapshotSources = new Set();
for (const file of (await Promise.all(snapshotRoots.map(walk))).flat()) {
  const source = file.replace(/^artifacts\/(?:chambre-full-redesign|chambre-resumption|chambre-continuation|chambre-signature|chambre-experience)\/before\/[^/]+\//, '');
  if (seenSnapshotSources.has(source)) continue;
  seenSnapshotSources.add(source);
  try {
...
const additions = ['components/enter/PublicChamberEntry.tsx', 'scripts/chambre-redesign-inventory.mjs',
  'components/v2/ChambreResonance.tsx', 'components/v2/chambre-signature.css',
  'tests/chambre-signature-foundation.test.mjs',
  'tests/chambre-signature-creation.test.mjs', 'tests/chambre-signature-music.test.mjs',
  'tests/chambre-signature-personal.test.mjs', 'scripts/chambre-signature-gallery.mjs',
  'docs/chambre-signature-validation.md', 'docs/chambre-signature-creation.md',
  'docs/chambre-signature-music.md', 'docs/chambre-signature-personal.md',
  ...
  'components/v2/experience-live-navigation.css', 'components/v2/experience-collection.css',
  'components/v2/experience-creation.css', 'components/v2/experience-account.css',
  'tests/experience-navigation-live.test.mjs', 'tests/experience-collection.test.mjs',
  'tests/experience-creation.test.mjs', 'tests/experience-account.test.mjs',
  'docs/experience-collection.md', 'docs/experience-creation.md', 'docs/experience-account.md'];
additions.push('docs/chambre-experience-validation.md', 'scripts/chambre-experience-gallery.mjs');
