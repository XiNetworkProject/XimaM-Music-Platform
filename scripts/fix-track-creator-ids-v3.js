...
async function fixTrackCreatorIdsV3() {
  console.log('🔧 Correction des creator_id manquants dans la table tracks (V3 - ObjectId)...');
        ...
        if (mongoTrack && mongoTrack.artist) {
          ...
          if (user && !userError) {
            ...
            try {
              ...
              const mongoUser = await db.collection('users').findOne({ _id: artistObjectId });
              if (mongoUser) {
              ...
              console.log(`   ❌ Erreur conversion artistId en ObjectId: ${artistId}`);
            }
          }
        } else {
