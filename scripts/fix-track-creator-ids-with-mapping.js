...
async function fixTrackCreatorIdsWithMapping() {
  console.log('🔧 Correction des creator_id avec mapping MongoDB → Supabase...');
  ...
  try {
    ...
    for (const mongoUser of mongoUsers) {
      ...
      if (supabaseUser) {
        idMapping.set(mongoUser._id.toString(), supabaseUser.id);
        console.log(`   ✅ ${mongoUsername} (${mongoUser._id}) → ${supabaseUser.id}`);
      } else {
    ...
    for (const track of tracksWithNullCreator) {
      ...
      try {
        ...
        if (mongoTrack && mongoTrack.artist) {
          ...
          const newSupabaseId = idMapping.get(mongoArtistId);
          ...
          if (newSupabaseId) {
            ...
            console.log(`   ❌ Aucun mapping trouvé pour l'artiste: ${mongoArtistId}`);
          }
        } else {
