...
async function fixTrackCreatorIds() {
  console.log('🔧 Correction des creator_id manquants dans la table tracks...');
  ...
  try {
    ...
    for (const track of tracksWithNullCreator) {
      ...
      if (mongoTrack && mongoTrack.artist) {
        ...
        const artistId = mongoTrack.artist._id || mongoTrack.artist;
