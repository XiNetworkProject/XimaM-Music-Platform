...
function normalizeMobileUser(value: MobileUser | null | undefined): MobileUser | null {
  ...
  return { ...value, avatar: toPublicMediaUrl(value.avatar) || null };
}
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const phoneLoginChallengeRef = useRef(new Map<string, string>());
  const phoneLinkChallengeRef = useRef(new Map<string, string>());
  ...
  const applySessionPayload = useCallback(async (
    ...
    await persistSession(
      ...
      normalizeMobileUser(payload.user),
  ...
  const requestPhoneCode = useCallback(async (phone: string) => {
    ...
    const normalizedPhone = String(json.data.phone);
    const challengeToken = typeof json.data.challengeToken === 'string' ? json.data.challengeToken : '';
    if (!challengeToken) throw new Error('Challenge SMS incomplet');
    phoneLoginChallengeRef.current.set(normalizedPhone, challengeToken);
  ...
  const verifyPhoneCode = useCallback(async (phone: string, code: string) => {
    const response = await authFetch('/api/auth/mobile/phone/verify', null, {
      method: 'POST',
      body: JSON.stringify({ phone, code, challengeToken: phoneLoginChallengeRef.current.get(phone) || '' }),
    });
    const json = await response.json().catch(() => null);
    const payload = json?.data as SessionPayload | undefined;
    if (!response.ok || !payload?.token || !payload?.user) {
    ...
    await applySessionPayload(payload);
    phoneLoginChallengeRef.current.delete(phone);
    await loadBiometricPreference(payload.user.id, false);
  }, [applySessionPayload, loadBiometricPreference]);
  ...
  const updateAccount = useCallback(async (input: AccountUpdateInput) => {
    ...
    const normalizedUser = normalizeMobileUser(details.user);
    setUser(normalizedUser);
    setMfaFactors(details.mfaFactors || []);
    if (normalizedUser) await AsyncStorage.setItem(USER_KEY, JSON.stringify(normalizedUser));
    return details;
  }, []);
  ...
  const requestPhoneLink = useCallback(async (phone: string) => {
    ...
    if (!normalizedPhone || !challengeToken) throw new Error('Challenge SMS incomplet');
    phoneLinkChallengeRef.current.set(normalizedPhone, challengeToken);
    return normalizedPhone;
  }, [postContact]);
  ...
  const verifyPhoneLink = useCallback(async (phone: string, code: string) => {
    const data = await postContact({
      ...
      phone,
      code,
      challengeToken: phoneLinkChallengeRef.current.get(phone) || '',
    });
    phoneLinkChallengeRef.current.delete(phone);
