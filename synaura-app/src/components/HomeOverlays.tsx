...
const searchSuggestions = [
  { label: 'En feu', icon: 'flame', tone: '#FF6B6B' },
  { label: 'Nouveaux talents', icon: 'sparkles', tone: '#8B5CF6' },
  { label: 'Electronic', icon: 'pulse', tone: '#22D3EE' },
  { label: 'Rap', icon: 'mic', tone: '#F5B84B' },
] as const;
...
export function NotificationModal({ visible, onClose, onUnreadChange }: { visible: boolean; onClose: () => void; onUnreadChange: (count: number) => void }) {
  ...
  return (
    <Modal visible={visible} animationType="fade" transparent onRequestClose={onClose}>
      <View style={styles.scrim}>
        <View style={[styles.sheet, styles.notificationSheet, { marginTop: insets.top + 8 }]}>
