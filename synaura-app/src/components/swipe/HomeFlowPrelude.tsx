...
function AuraBackground() {
  return (
    <Svg pointerEvents="none" style={StyleSheet.absoluteFillObject} viewBox="0 0 100 100" preserveAspectRatio="none">
      <Defs>
        <SvgRadialGradient id="violetAura" cx="8" cy="4" rx="72" ry="62" gradientUnits="userSpaceOnUse">
          <Stop offset="0" stopColor="#7357C6" stopOpacity="0.48" />
          <Stop offset="0.34" stopColor="#7357C6" stopOpacity="0.2" />
          <Stop offset="0.72" stopColor="#7357C6" stopOpacity="0.045" />
          <Stop offset="1" stopColor="#7357C6" stopOpacity="0" />
        </SvgRadialGradient>
        <SvgRadialGradient id="coralAura" cx="98" cy="12" rx="68" ry="58" gradientUnits="userSpaceOnUse">
          <Stop offset="0" stopColor="#D96D63" stopOpacity="0.4" />
          <Stop offset="0.38" stopColor="#D96D63" stopOpacity="0.17" />
          <Stop offset="0.76" stopColor="#D96D63" stopOpacity="0.035" />
          <Stop offset="1" stopColor="#D96D63" stopOpacity="0" />
        </SvgRadialGradient>
        <SvgRadialGradient id="cyanAura" cx="52" cy="18" rx="55" ry="48" gradientUnits="userSpaceOnUse">
          <Stop offset="0" stopColor="#4A9EAA" stopOpacity="0.25" />
          <Stop offset="0.42" stopColor="#4A9EAA" stopOpacity="0.1" />
          <Stop offset="0.82" stopColor="#4A9EAA" stopOpacity="0.02" />
          <Stop offset="1" stopColor="#4A9EAA" stopOpacity="0" />
        </SvgRadialGradient>
      </Defs>
      <Rect x="0" y="0" width="100" height="100" fill="url(#violetAura)" />
      <Rect x="0" y="0" width="100" height="100" fill="url(#coralAura)" />
      <Rect x="0" y="0" width="100" height="100" fill="url(#cyanAura)" />
    </Svg>
  );
}

function CardAura({ color, opacity = 0.3 }: { color: string; opacity?: number }) {
  return (
    <Svg pointerEvents="none" style={StyleSheet.absoluteFillObject} viewBox="0 0 100 100" preserveAspectRatio="none">
      <Defs>
        <SvgRadialGradient id="cardAura" cx="94" cy="4" rx="72" ry="82" gradientUnits="userSpaceOnUse">
          <Stop offset="0" stopColor={color} stopOpacity={opacity} />
          <Stop offset="0.42" stopColor={color} stopOpacity={opacity * 0.36} />
          <Stop offset="0.78" stopColor={color} stopOpacity={opacity * 0.06} />
          <Stop offset="1" stopColor={color} stopOpacity="0" />
        </SvgRadialGradient>
      </Defs>
      <Rect x="0" y="0" width="100" height="100" fill="url(#cardAura)" />
    </Svg>
  );
}
...
export function HomeFlowPrelude(props: Props) {
  ...
  useEffect(() => {
    ...
    const auraLoop = Animated.loop(Animated.sequence([
      Animated.timing(auraDrift, { toValue: 1, duration: 6200, easing: Easing.inOut(Easing.sin), useNativeDriver: true }),
      Animated.timing(auraDrift, { toValue: 0, duration: 6200, easing: Easing.inOut(Easing.sin), useNativeDriver: true }),
    ]));
  ...
  const auraOpacity = auraDrift.interpolate({ inputRange: [0, 1], outputRange: [0.82, 1] });
  const auraTranslateX = auraDrift.interpolate({ inputRange: [0, 1], outputRange: [-8, 8] });

  const renderRailItem = ({ item }: { item: RailItem; index: number }) => {
    const width = item.kind === 'shortcut' ? Math.max(148, cardWidth * 0.66) : cardWidth;
    const itemStyle = [styles.railCard, { width, height: railHeight }];

    if (item.kind === 'post') {
      const post = item.post;
      return (
        <MotionPressable accessibilityLabel={post ? `Ouvrir le post de ${post.author}` : 'Découvrir la communauté'} onPress={() => post ? hapticPress(() => onOpenPost(post)) : hapticPress(onDiscover)} style={itemStyle} scaleTo={0.975}>
          <LinearGradient colors={['rgba(217,109,99,0.25)', 'rgba(31,24,29,0.97)', 'rgba(244,162,97,0.1)']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={StyleSheet.absoluteFillObject} />
          <CardAura color="#D96D63" opacity={0.32} />
          <View style={styles.cardHeader}>
            <View style={styles.cardKickerRow}><View style={[styles.liveDot, { backgroundColor: '#D96D63' }]} /><Text style={[styles.cardKicker, { color: '#F0AAA2' }]}>ÇA BOUGE MAINTENANT</Text></View>
            <Ionicons name="trending-up" size={14} color="#F4A261" />
          </View>
          <View style={styles.postBody}>
            <View style={styles.postAvatarRing}>{post?.avatar ? <ExpoImage source={{ uri: post.avatar }} contentFit="cover" transition={140} style={styles.postAvatar} /> : <Image source={brandSymbol} resizeMode="contain" style={styles.postAvatarFallback} />}</View>
          ...
          <View style={styles.cardFooter}>
            <View style={styles.cardCtaRow}><Text style={styles.cardCta}>{post ? 'Voir ce que t’as raté' : 'Explorer la communauté'}</Text><Ionicons name="chevron-forward" size={13} color="rgba(255,255,255,0.66)" /></View>
            {post ? <View style={styles.postCounts}><Ionicons name="heart-outline" size={11} color="rgba(255,255,255,0.38)" /><Text style={styles.postCountText}>{compactCount(post.likesCount)}</Text><Ionicons name="chatbubble-outline" size={11} color="rgba(255,255,255,0.38)" /><Text style={styles.postCountText}>{compactCount(post.commentsCount)}</Text></View> : null}
          </View>
        </MotionPressable>
      );
    }
    ...
    if (item.kind === 'social') {
      ...
      return (
        <MotionPressable accessibilityLabel="Ouvrir ton réseau" onPress={() => hapticPress(onDiscover)} style={itemStyle} scaleTo={0.975}>
          <LinearGradient colors={['rgba(115,87,198,0.29)', 'rgba(24,20,31,0.97)', 'rgba(74,158,170,0.1)']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={StyleSheet.absoluteFillObject} />
          <CardAura color="#7357C6" opacity={0.34} />
    ...
    if (item.kind === 'track') {
      ...
      return (
        <MotionPressable accessibilityLabel={`Ouvrir ${item.track.title}`} onPress={() => hapticPress(() => onOpenTrack(item.track))} style={itemStyle} scaleTo={0.975}>
          ...
          <LinearGradient colors={['rgba(5,5,7,0.04)', 'rgba(5,5,7,0.18)', 'rgba(5,5,7,0.94)']} locations={[0, 0.38, 1]} style={StyleSheet.absoluteFillObject} />
          <LinearGradient colors={['rgba(5,5,7,0.64)', 'transparent']} start={{ x: 0, y: 0.5 }} end={{ x: 1, y: 0.5 }} style={StyleSheet.absoluteFillObject} />
          <View style={styles.trackCardContent}><Text style={styles.trackBadge}>{item.badge}</Text><View><Text numberOfLines={2} style={styles.trackCardTitle}>{item.track.title}</Text><Text numberOfLines={1} style={styles.trackCardArtist}>{artistName(item.track)}</Text><View style={styles.trackMeta}><Ionicons name="headset-outline" size={12} color="rgba(255,255,255,0.76)" /><Text style={styles.trackMetaText}>{compactCount(item.track.plays || 0)} écoutes</Text></View></View></View>
        </MotionPressable>
      );
    }
    ...
    if (item.kind === 'shortcut') {
      const { shortcut } = item;
      return (
        <MotionPressable accessibilityLabel={shortcut.label} onPress={() => hapticPress(shortcut.onPress)} style={[...itemStyle, { borderColor: `${shortcut.accent}55` }]} scaleTo={0.97}>
          <LinearGradient colors={[`${shortcut.accent}32`, 'rgba(22,20,26,0.98)', `${shortcut.accent}0D`]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={StyleSheet.absoluteFillObject} />
          <CardAura color={shortcut.accent} opacity={0.24} />
          <View style={styles.shortcutCardContent}><View style={[styles.shortcutIcon, { backgroundColor: `${shortcut.accent}26`, borderColor: `${shortcut.accent}42` }]}><Ionicons name={shortcut.icon} size={19} color={shortcut.accent} /></View><View><Text style={styles.shortcutTitle}>{shortcut.label}</Text><Text numberOfLines={1} style={styles.shortcutSub}>{shortcut.sub}</Text></View></View>
        </MotionPressable>
      );
    }

    return (
      <MotionPressable accessibilityLabel="Ouvrir le Studio IA" onPress={() => hapticPress(onStudio)} style={itemStyle} scaleTo={0.975}>
        <LinearGradient colors={['rgba(217,109,99,0.28)', 'rgba(115,87,198,0.2)', 'rgba(18,16,23,0.98)']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={StyleSheet.absoluteFillObject} />
        <CardAura color="#D96D63" opacity={0.3} />
        <View style={styles.studioIcon}><Ionicons name="flash-outline" size={21} color="#FFD8D3" /></View><Text numberOfLines={3} style={styles.studioTitle}>Ton prochain banger attend juste un clic.</Text><View style={styles.studioCtaRow}><Text style={styles.studioCta}>Va cuisiner ça</Text><Ionicons name="chevron-forward" size={13} color="#F0AAA2" /></View>
      </MotionPressable>
    );
  };
  ...
  return (
    <Animated.View {...panResponder.panHandlers} style={[styles.overlay, { opacity: screenOpacity, transform: [{ translateY: screenTranslateY }] }]}>
      <View style={[styles.surface, { paddingBottom: bottomPad }]}>
        ...
        <Animated.View pointerEvents="none" style={[StyleSheet.absoluteFillObject, { opacity: auraOpacity, transform: [{ translateX: auraTranslateX }, { scale: 1.08 }] }]}><AuraBackground /></Animated.View>
        <Animated.View style={[styles.stage, { transform: [{ translateY: contentTranslateY }, { scale: contentScale }] }]}>
          <View style={[styles.header, { minHeight: topPad + (compact ? 50 : 56), paddingTop: topPad + 6, paddingHorizontal: responsive.gutter }]}>
            <MotionPressable accessibilityLabel="Entrer dans le Flow" onPress={finish} style={styles.brandRow} scaleTo={0.97}><View style={styles.brandLogo}><Animated.View style={[styles.brandPulse, { opacity: pulseOpacity, transform: [{ scale: pulseScale }] }]} /><Image source={brandSymbol} resizeMode="contain" style={styles.brandImage} /></View><View style={styles.brandCopy}><Text style={styles.brandName}>Synaura</Text><Text numberOfLines={1} style={styles.brandLine}>{greetingName ? `Salut ${greetingName}, regarde ce que t’as raté` : 'Écoute, crée, partage'}</Text></View></MotionPressable>
            <View style={styles.headerActions}><MotionPressable accessibilityLabel="Rechercher" onPress={() => hapticPress(onSearch)} style={styles.headerButton} scaleTo={0.9}><Ionicons name="search" size={compact ? 17 : 18} color="rgba(255,255,255,0.82)" /></MotionPressable><MotionPressable accessibilityLabel="Messages" onPress={() => hapticPress(() => navigation.navigate('Messages'))} style={styles.headerButton} scaleTo={0.9}><Ionicons name="chatbubble-outline" size={compact ? 17 : 18} color="rgba(255,255,255,0.82)" /></MotionPressable><MotionPressable accessibilityLabel="Notifications" onPress={() => hapticPress(onNotifications)} style={styles.headerButton} scaleTo={0.9}><Ionicons name="notifications-outline" size={compact ? 18 : 19} color="rgba(255,255,255,0.82)" /></MotionPressable></View>
          </View>
          <View style={[styles.pulseSection, { height: pulseHeight, paddingHorizontal: responsive.gutter }]}>
            <LinearGradient colors={['rgba(20,18,26,0.96)', 'rgba(12,11,15,0.94)', 'rgba(9,9,11,0.9)']} locations={[0, 0.56, 1]} style={styles.pulseCard}>
              <CardAura color="#7357C6" opacity={0.12} />
              <View style={styles.pulseHeader}><View style={styles.badgeRow}><View style={[styles.badge, styles.badgeViolet]}><Text style={[styles.badgeText, { color: '#DCCEFF' }]}>PENDANT TON ABSENCE</Text></View><View style={[styles.badge, styles.badgeCyan]}><Text style={[styles.badgeText, { color: '#A8DEE5' }]}>SWIPE LES CARTES</Text></View></View><Text maxFontSizeMultiplier={1.08} numberOfLines={3} adjustsFontSizeToFit style={[styles.punchline, compact && styles.punchlineCompact]}>{PUNCHLINES[phraseIndex]}</Text></View>
              <View style={styles.ticker}><LinearGradient colors={['rgba(115,87,198,0.16)', 'rgba(255,255,255,0.055)', 'rgba(74,158,170,0.14)']} start={{ x: 0, y: 0.5 }} end={{ x: 1, y: 0.5 }} style={StyleSheet.absoluteFillObject} /><Animated.View key={bannerIndex} style={[styles.tickerContent, { opacity: bannerIn, transform: [{ translateY: bannerIn.interpolate({ inputRange: [0, 1], outputRange: [6, 0] }) }] }]}><View style={styles.tickerIcon}><Ionicons name="flash-outline" size={13} color="#A8DEE5" /></View>{!responsive.isTiny ? <Text style={styles.tickerLabel}>EN CE MOMENT</Text> : null}<Text numberOfLines={1} style={styles.tickerMessage}>{bannerItems[bannerIndex]}</Text><Text style={styles.tickerCount}>{String(bannerIndex + 1).padStart(2, '0')} / {String(bannerItems.length).padStart(2, '0')}</Text></Animated.View><Animated.View style={[styles.tickerProgress, { width: bannerWidth }]} /></View>
              <Animated.View style={[styles.railWrap, { flex: 1, opacity: cardsIn, transform: [{ translateY: cardsTranslateY }] }]}><LinearGradient pointerEvents="none" colors={['#100F14', 'rgba(16,15,20,0)']} start={{ x: 0, y: 0.5 }} end={{ x: 1, y: 0.5 }} style={styles.leftFade} /><LinearGradient pointerEvents="none" colors={['rgba(16,15,20,0)', '#100F14']} start={{ x: 0, y: 0.5 }} end={{ x: 1, y: 0.5 }} style={styles.rightFade} /><FlatList horizontal data={railItems} keyExtractor={(item) => item.id} renderItem={renderRailItem} ItemSeparatorComponent={() => <View style={styles.railGap} />} showsHorizontalScrollIndicator={false} contentContainerStyle={styles.railContent} snapToInterval={cardWidth + 10} decelerationRate="fast" disableIntervalMomentum onMomentumScrollEnd={() => { void Haptics.selectionAsync().catch(() => {}); }} /></Animated.View>
            </LinearGradient>
          </View>
          <View style={styles.flowPreview}>
            {featuredCover ? <Animated.View style={[StyleSheet.absoluteFillObject, { transform: [{ scale: coverScale }] }]}><ExpoImage source={{ uri: featuredCover }} contentFit="cover" transition={180} cachePolicy="memory-disk" style={StyleSheet.absoluteFillObject} /></Animated.View> : <LinearGradient colors={['#2C1830', '#143239', '#0A0A0D']} style={StyleSheet.absoluteFillObject} />}
            <LinearGradient colors={['rgba(6,6,8,0.14)', 'rgba(6,6,8,0.06)', 'rgba(6,6,8,0.78)', 'rgba(6,6,8,0.98)']} locations={[0, 0.32, 0.78, 1]} style={StyleSheet.absoluteFillObject} /><LinearGradient colors={['rgba(6,6,8,0.62)', 'rgba(6,6,8,0.08)', 'rgba(6,6,8,0.02)']} start={{ x: 0, y: 0.5 }} end={{ x: 1, y: 0.5 }} style={StyleSheet.absoluteFillObject} />
            <View style={styles.previewBadge}><View style={styles.previewDot} /><Text style={styles.previewBadgeText}>APERÇU DU FLOW</Text></View>
            <View style={[styles.flowChips, { left: responsive.gutter }]}><MotionPressable accessibilityLabel="Pour toi" onPress={onDiscover} style={styles.chip} scaleTo={0.95}><Text style={styles.chipText}>Pour toi</Text></MotionPressable><MotionPressable accessibilityLabel="Ça monte" onPress={onRadar} style={[styles.chip, styles.chipHot]} scaleTo={0.95}><Text style={[styles.chipText, { color: '#FFD4CE' }]}>Ça monte</Text></MotionPressable></View>
            <View style={[styles.flowCopy, { left: responsive.gutter, maxWidth: responsive.safeWidth * (responsive.isTiny ? 0.61 : 0.68) }]}><Text style={styles.flowKicker}>{isCurrentTrack ? 'REPRENDRE MAINTENANT' : 'PREMIER SON DE TON FLOW'}</Text><MotionPressable accessibilityLabel="Ouvrir le morceau" disabled={!featuredTrack} onPress={() => featuredTrack && hapticPress(() => onOpenTrack(featuredTrack))} style={styles.trackCopy} scaleTo={0.99}><Text maxFontSizeMultiplier={1.08} numberOfLines={2} adjustsFontSizeToFit style={[styles.flowTitle, compact && styles.flowTitleCompact]}>{featuredTrack?.title || 'Ton Flow se prépare'}</Text><Text numberOfLines={1} style={styles.flowArtist}>{featuredTrack ? artistName(featuredTrack) : 'Synaura prépare ta sélection'}</Text></MotionPressable>{featuredTrack ? <View style={styles.flowMeta}><Text style={styles.flowMetaText}>{compactCount(featuredTrack.plays || 0)} écoutes</Text><View style={styles.metaDot} /><Text style={styles.flowMetaText}>{compactCount(countOf(featuredTrack.likesCount ?? featuredTrack.likes))} j’aime</Text>{nextTrack && !responsive.isTiny ? <Text numberOfLines={1} style={styles.nextTrack}>Ensuite : {nextTrack.title}</Text> : null}</View> : null}<View style={styles.flowButtons}><MotionPressable accessibilityLabel="Écouter" disabled={!featuredTrack} onPress={() => featuredTrack && hapticPress(() => onPlayTrack(featuredTrack))} style={[styles.playButton, !featuredTrack && styles.disabledButton]} scaleTo={0.92}><Animated.View style={[styles.playPulse, { opacity: pulseOpacity, transform: [{ scale: pulseScale }] }]} />{isPlayingFeatured ? <View style={styles.equalizer}>{[0, 1, 2, 3].map((bar) => <Animated.View key={bar} style={[styles.eqBar, { transform: [{ scaleY: equalizer.interpolate({ inputRange: [0, 1], outputRange: bar % 2 ? [1, 0.38] : [0.38, 1] }) }] }]} />)}</View> : <Ionicons name="play" size={compact ? 21 : 23} color="#111111" style={styles.playIcon} />}</MotionPressable><MotionPressable accessibilityLabel="Voir en plein écran" onPress={finish} style={styles.fullscreenButton} scaleTo={0.96}><Ionicons name="radio-outline" size={16} color="#FFFFFF" /><Text numberOfLines={1} adjustsFontSizeToFit style={styles.fullscreenText}>Voir en plein écran</Text></MotionPressable></View></View>
            <View style={[styles.flowActions, { right: responsive.gutter }]}><MotionPressable accessibilityLabel="Aimer" disabled={!featuredTrack} onPress={() => featuredTrack && hapticPress(() => onOpenTrack(featuredTrack))} style={styles.actionButton} scaleTo={0.9}><Ionicons name="heart-outline" size={compact ? 21 : 23} color="#FFFFFF" /></MotionPressable><Text style={styles.actionCount}>{featuredTrack ? compactCount(countOf(featuredTrack.likesCount ?? featuredTrack.likes)) : '0'}</Text><MotionPressable accessibilityLabel="Commentaires" disabled={!featuredTrack} onPress={() => featuredTrack && hapticPress(() => onOpenTrack(featuredTrack))} style={styles.actionButton} scaleTo={0.9}><Ionicons name="chatbubble-outline" size={compact ? 20 : 22} color="#FFFFFF" /></MotionPressable><Text style={styles.actionCount}>{featuredTrack ? compactCount(countOf(featuredTrack.commentsCount ?? featuredTrack.comments)) : '0'}</Text><MotionPressable accessibilityLabel="Partager" disabled={!featuredTrack} onPress={() => featuredTrack && hapticPress(() => onOpenTrack(featuredTrack))} style={styles.actionButton} scaleTo={0.9}><Ionicons name="share-social-outline" size={compact ? 20 : 22} color="#FFFFFF" /></MotionPressable></View>
            <MotionPressable accessibilityLabel="Glisser vers le haut pour ouvrir le Flow" onPress={finish} style={[styles.swipePill, { right: responsive.gutter + (compact ? 47 : 54), top: compact ? '29%' : '31%' }]} scaleTo={0.97}><LinearGradient colors={['rgba(115,87,198,0.34)', 'rgba(74,158,170,0.12)', 'rgba(217,109,99,0.24)']} start={{ x: 0.5, y: 0 }} end={{ x: 0.5, y: 1 }} style={StyleSheet.absoluteFillObject} /><View style={styles.chevrons}><Ionicons name="chevron-up" size={14} color="#DCCEFF" /><Ionicons name="chevron-up" size={14} color="#A8DEE5" style={styles.chevronOverlap} /><Ionicons name="chevron-up" size={14} color="#F0AAA2" style={styles.chevronOverlap} /></View><View style={styles.fingerTrack}><Animated.View style={[styles.finger, { opacity: fingerOpacity, transform: [{ translateY: fingerTranslateY }] }]}><View style={styles.fingerDot} /></Animated.View></View><Text style={styles.swipeText}>GLISSE</Text></MotionPressable>
          </View>
        </Animated.View>
      </View>
    </Animated.View>
...
const styles = StyleSheet.create({
  ...
  brandName: { color: '#F7F6F3', fontFamily: FONT_BLACK, fontSize: 18, lineHeight: 19, fontWeight: '900', letterSpacing: -0.4 },
  ...
  postName: { color: 'rgba(255,255,255,0.95)', fontFamily: FONT_BLACK, fontSize: 13.5, fontWeight: '900', letterSpacing: -0.25 },
  ...
  cardCta: { flexShrink: 1, color: 'rgba(255,255,255,0.72)', fontFamily: FONT_BLACK, fontSize: 8.5, fontWeight: '900' },
  postCounts: { flexDirection: 'row', alignItems: 'center', gap: 3 },
  postCountText: { color: 'rgba(255,255,255,0.36)', fontFamily: FONT_BLACK, fontSize: 7.5, fontWeight: '900' },
  socialIcon: { width: 34, height: 34, borderRadius: 11, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(115,87,198,0.26)', borderWidth: 1, borderColor: 'rgba(169,139,232,0.3)' },
  socialBadge: { alignSelf: 'flex-start', borderRadius: 999, borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)', backgroundColor: 'rgba(0,0,0,0.2)', paddingHorizontal: 9, paddingVertical: 5, color: 'rgba(255,255,255,0.6)', fontFamily: FONT_BLACK, fontSize: 7.5, fontWeight: '900', letterSpacing: 0.8 },
  ...
  stackAvatarWrap: { width: 25, height: 25, borderRadius: 13, overflow: 'hidden', alignItems: 'center', justifyContent: 'center', backgroundColor: '#17151B', borderWidth: 1.5, borderColor: 'rgba(255,255,255,0.7)' },
  stackAvatar: { width: '100%', height: '100%' },
  stackAvatarFallback: { width: 15, height: 15 },
  socialHint: { color: 'rgba(255,255,255,0.42)', fontFamily: FONT_BOLD, fontSize: 8, fontWeight: '700' },
  trackCardContent: { flex: 1, justifyContent: 'space-between' },
  trackBadge: { alignSelf: 'flex-start', overflow: 'hidden', borderRadius: 999, borderWidth: 1, borderColor: 'rgba(114,187,197,0.28)', backgroundColor: 'rgba(74,158,170,0.17)', paddingHorizontal: 8, paddingVertical: 5, color: '#A8DEE5', fontFamily: FONT_BLACK, fontSize: 7.5, fontWeight: '900', letterSpacing: 0.75, textTransform: 'uppercase' },
  trackCardTitle: { color: '#FFFFFF', fontFamily: FONT_BLACK, fontSize: 15.5, lineHeight: 16.5, fontWeight: '900', letterSpacing: -0.4 },
  trackCardArtist: { marginTop: 3, color: 'rgba(255,255,255,0.56)', fontFamily: FONT_BOLD, fontSize: 9, fontWeight: '700' },
  trackMeta: { marginTop: 5, flexDirection: 'row', alignItems: 'center', gap: 4 },
  trackMetaText: { color: 'rgba(255,255,255,0.6)', fontFamily: FONT_BLACK, fontSize: 8, fontWeight: '900' },
  ...
  shortcutSub: { marginTop: 3, color: 'rgba(255,255,255,0.42)', fontFamily: FONT_BOLD, fontSize: 8.5, fontWeight: '700' },
  studioIcon: { width: 38, height: 38, borderRadius: 12, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(217,109,99,0.22)', borderWidth: 1, borderColor: 'rgba(240,170,162,0.28)' },
  ...
  flowPreview: { flex: 1, minHeight: 250, overflow: 'hidden', borderTopLeftRadius: 30, borderTopRightRadius: 30, borderTopWidth: 1, borderColor: 'rgba(255,255,255,0.15)', backgroundColor: '#121014', shadowColor: '#000000', shadowOpacity: 0.38, shadowRadius: 26, shadowOffset: { width: 0, height: -10 }, elevation: 8 },
  previewBadge: { position: 'absolute', zIndex: 8, top: 13, alignSelf: 'center', minHeight: 31, flexDirection: 'row', alignItems: 'center', gap: 8, borderRadius: 16, paddingHorizontal: 13, backgroundColor: 'rgba(12,10,14,0.54)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.18)' },
  previewDot: { width: 7, height: 7, borderRadius: 4, backgroundColor: '#72BBC5', shadowColor: '#72BBC5', shadowOpacity: 0.9, shadowRadius: 8, elevation: 3 },
  previewBadgeText: { color: 'rgba(255,255,255,0.86)', fontFamily: FONT_BLACK, fontSize: 8.5, fontWeight: '900', letterSpacing: 0.85 },
  flowChips: { position: 'absolute', zIndex: 8, top: 53, flexDirection: 'row', gap: 7 },
  chip: { minHeight: 31, justifyContent: 'center', borderRadius: 16, paddingHorizontal: 12, backgroundColor: 'rgba(7,7,9,0.42)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.16)' },
  chipHot: { backgroundColor: 'rgba(217,109,99,0.18)', borderColor: 'rgba(217,109,99,0.34)' },
  chipText: { color: 'rgba(255,255,255,0.84)', fontFamily: FONT_BLACK, fontSize: 9, fontWeight: '900' },
  ...
  flowArtist: { marginTop: 6, color: 'rgba(255,255,255,0.66)', fontFamily: FONT_BLACK, fontSize: 10.5, fontWeight: '900' },
  flowMeta: { marginTop: 7, flexDirection: 'row', alignItems: 'center', gap: 7 },
  flowMetaText: { color: 'rgba(255,255,255,0.54)', fontFamily: FONT_BLACK, fontSize: 8.5, fontWeight: '900' },
  metaDot: { width: 3, height: 3, borderRadius: 2, backgroundColor: 'rgba(255,255,255,0.3)' },
  nextTrack: { maxWidth: 120, color: 'rgba(255,255,255,0.32)', fontFamily: FONT_BOLD, fontSize: 8, fontWeight: '700' },
  flowButtons: { marginTop: 12, flexDirection: 'row', alignItems: 'center', gap: 9 },
  playButton: { width: 55, height: 55, borderRadius: 28, alignItems: 'center', justifyContent: 'center', backgroundColor: '#F7F6F3', shadowColor: '#000000', shadowOpacity: 0.38, shadowRadius: 18, shadowOffset: { width: 0, height: 8 }, elevation: 8 },
  playPulse: { position: 'absolute', width: 55, height: 55, borderRadius: 28, borderWidth: 1, borderColor: 'rgba(255,255,255,0.72)' },
  playIcon: { marginLeft: 2 },
  equalizer: { height: 19, flexDirection: 'row', alignItems: 'flex-end', gap: 2 },
  eqBar: { width: 3, height: 18, borderRadius: 2, backgroundColor: '#7357C6' },
  fullscreenButton: { minWidth: 0, maxWidth: 190, height: 50, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, borderRadius: 16, paddingHorizontal: 16, backgroundColor: 'rgba(8,8,10,0.46)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.2)' },
  ...
  actionButton: { width: 47, height: 47, borderRadius: 24, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(7,7,9,0.43)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.2)' },
  actionCount: { color: 'rgba(255,255,255,0.7)', fontFamily: FONT_BLACK, fontSize: 8, fontWeight: '900' },
  swipePill: { position: 'absolute', zIndex: 13, width: 60, height: 154, overflow: 'hidden', alignItems: 'center', borderRadius: 25, borderWidth: 1, borderColor: 'rgba(255,255,255,0.22)', backgroundColor: 'rgba(8,8,10,0.48)', paddingTop: 11, paddingBottom: 10 },
  ...
  finger: { position: 'absolute', bottom: 4, width: 18, height: 39, borderRadius: 10, alignItems: 'center', paddingTop: 5, backgroundColor: 'rgba(255,255,255,0.1)', borderWidth: 2, borderColor: 'rgba(255,255,255,0.84)', shadowColor: '#000000', shadowOpacity: 0.28, shadowRadius: 10, shadowOffset: { width: 0, height: 5 }, elevation: 4 },
  fingerDot: { width: 5, height: 5, borderRadius: 3, backgroundColor: '#DCCEFF', shadowColor: '#A98BE8', shadowOpacity: 0.9, shadowRadius: 7, elevation: 3 },
  swipeText: { color: 'rgba(255,255,255,0.65)', fontFamily: FONT_BLACK, fontSize: 7.5, fontWeight: '900', letterSpacing: 0.8 },
});
