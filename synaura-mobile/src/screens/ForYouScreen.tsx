...
export default function ForYouScreen() {
  ...
  const openPlayer = useCallback(
    (index: number) => {
      ...
      (parent || navigation).navigate("Player", { tracks, startIndex: index, title: "Pour toi" });
    },
    [navigation, setQueueAndPlay, tracks]
  );

  const list = useMemo(() => tracks.slice(0, 80), [tracks]);
    ...
    <View style={styles.screen}>
      ...
      <View style={styles.safeArea}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>Pour toi</Text>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 220 }}>
          {loading ? (
            ...
            <View style={{ gap: 8 }}>
              {list.map((t, idx) => (
                <TrackRow key={t._id} track={t} onPress={() => openPlayer(idx)} />
              ))}
            </View>
          )}
        </ScrollView>
