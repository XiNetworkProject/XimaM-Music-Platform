...
test('the redesigned lead and opening retain real playback and early navigation; pre-existing music CSS remains untouched', async () => {
  ...
  // The user requested a new composition. Replace only obsolete JSX freezes;
  // the separate exact logic, ordering and native-action hashes above remain unchanged.
  assert.match(print(lead, ast), /TrackCover[\s\S]*trackId=\{track._id\}/);
  assert.match(print(lead, ast), /onClick=\{toggle\}/);
  assert.match(print(lead, ast), /experience-discover-feature-topline/);
  const openings = [];
  function visit(node) {
  ...
  assert.match(print(openings[0], ast), /Le prochain/);
  assert.match(print(openings[0], ast), /href="#ambiances"/);
  assert.match(print(openings[0], ast), /DiscoverLeadCard track=\{leadTrack\} queue=\{leadQueue\} totalTracks=\{totalTracks\}/);
