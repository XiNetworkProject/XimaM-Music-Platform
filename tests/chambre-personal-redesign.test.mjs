...
export function behaviorFingerprint(source, fileName) {
  ...
  // actual rewritten controls are executed in experience-account.test.mjs.
  // Every other route retains the original expression-order fingerprint.
  if (['app/settings/SettingsClient.tsx', 'app/subscriptions/page.tsx'].includes(fileName)) {
    return accountBehaviorFingerprint(source, fileName);
  }
  ...
  // The user explicitly requested an entirely new Library composition. These
  // exact display-only additions are mapped to its historical expression stream;
  // imports, non-JSX statements, handlers, keys and data-forwarding props still
  // use the original strict fingerprint. Other routes and before snapshots do
  // not enter this adapter. The actual new count/callback rendering is exercised
  // separately in experience-collection.test.mjs against the preserved snapshot.
  const collectionPresentation = fileName === 'app/library/LibraryClient.tsx' && source.includes('experience-collection-header');
  const collectionSeen = new Map();
  const recordPresentation = key => collectionSeen.set(key, (collectionSeen.get(key) || 0) + 1);
  const elementHasClass = (node, name) => ts.isJsxElement(node) && node.openingElement.attributes.properties.some(attribute => ts.isJsxAttribute(attribute) && attribute.name.text === 'className' && ts.isStringLiteral(attribute.initializer) && attribute.initializer.text.split(/\s+/).includes(name));
  const insideClass = (node, name) => {
    for (let current = node.parent; current; current = current.parent) if (elementHasClass(current, name)) return true;
    return false;
  };
  const collectionCopy = new Set([
    "search ? 'Aucun dossier ne correspond.' : 'Une collection commence par un son.'",
    ...
    "search || genreFilter !== 'all' ? 'Aucun favori dans cette sélection.' : 'Les coups de cœur se gardent ici.'",
    "search || genreFilter !== 'all' ? 'Ajustez votre recherche ou le filtre de genre.' : 'Ajoutez un morceau à vos favoris pour le retrouver en un instant.'",
    "search || genreFilter !== 'all' ? 'Aucune écoute dans cette sélection.' : 'Votre prochaine écoute commence ici.'",
    "search || genreFilter !== 'all' ? 'Ajustez votre recherche ou le filtre de genre.' : 'Les morceaux écoutés apparaîtront ici pour reprendre votre exploration.'",
  ]);
  const collectionCounts = ['playlists.length', 'favoriteTracks.length', 'recentTracks.length', 'downloadedTracks.length', 'upNextTracks.length'];
  const visit = (node) => {
    if (presentationAttribute(node)) return;
    if (ts.isJsxExpression(node) && node.expression) {
      const value = normalize(node.expression);
      if (collectionPresentation) {
        if (insideClass(node, 'experience-collection-count')) {
          assert.ok(collectionCounts.includes(value), `Unapproved Library counter expression: ${value}`);
          recordPresentation(`count:${value}`);
          return;
        }
        if ((insideClass(node, 'signature-library-empty') || insideClass(node, 'experience-collection-empty-state')) && collectionCopy.has(value)) {
          recordPresentation(`copy:${value}`);
          return;
        }
        if (insideClass(node, 'experience-collection-empty-state') && ts.isJsxAttribute(node.parent) && node.parent.name.text === 'size' && value === '28') {
          recordPresentation('empty-icon-size');
          return;
        }
        if (insideClass(node, 'experience-collection-sleeve-label') && value === 'playlist.isPublic ? null : null') {
          assert.match(node.expression.getText(file), /^playlist\.isPublic \? <Globe size=\{12\} \/> : <Lock size=\{12\} \/>$/);
          recordPresentation('privacy-icon');
          return;
        }
        if (insideClass(node, 'experience-collection-total')) {
          assert.equal(value, 'visiblePlaylists.length');
          expressions.push(value, "visiblePlaylists.length > 1 ? 's' : ''");
          recordPresentation('folder-total');
          return;
        }
      }
  ...
  visit(file);
  if (collectionPresentation) {
    for (const value of collectionCounts) assert.equal(collectionSeen.get(`count:${value}`), 1, value);
    for (const value of collectionCopy) assert.equal(collectionSeen.get(`copy:${value}`), 1, value);
    assert.equal(collectionSeen.get('empty-icon-size'), 2);
    assert.equal(collectionSeen.get('privacy-icon'), 1);
    assert.equal(collectionSeen.get('folder-total'), 1);
  }
