...
function reviewedPublishNavigation(ast) {
  // The experience redesign adds exactly these three links to existing routes.
  // Assert literal destinations, unique exact markers, Link tags and direct
  // release-path ancestry before omitting only their href attributes from the
  // historical fingerprint. Every other attribute and call stays audited.
  const additions = [
    { marker: 'experience-release-path experience-release-clip', href: '/clips/new' },
    { marker: 'experience-release-path experience-release-post', href: '/posts?compose=true' },
    { marker: 'experience-release-library', href: '/ai-library' },
  ];
  const attribute = (opening, name) => opening.attributes.properties.filter(node => ts.isJsxAttribute(node) && node.name.getText(ast) === name);
  const literal = node => node?.initializer && ts.isStringLiteral(node.initializer) ? node.initializer.text : null;
  const candidates = [];
  const visit = node => {
    if (ts.isJsxOpeningElement(node) || ts.isJsxSelfClosingElement(node)) candidates.push(node);
    ts.forEachChild(node, visit);
  };
  visit(ast);
  return new Set(additions.map(({ marker, href }) => {
    const matches = candidates.filter(node => attribute(node, 'className').some(value => literal(value) === marker));
    assert.equal(matches.length, 1, `Exactly one reviewed Publish marker: ${marker}`);
    const opening = matches[0];
    assert.equal(opening.tagName.getText(ast), 'Link', marker);
    const parent = ts.isJsxOpeningElement(opening) ? opening.parent.parent : opening.parent;
    assert.ok(ts.isJsxElement(parent), `${marker}: direct release-path child`);
    assert.equal(parent.openingElement.tagName.getText(ast), 'section', marker);
    assert.equal(literal(attribute(parent.openingElement, 'className')[0]), 'experience-release-paths', marker);
    const hrefs = attribute(opening, 'href');
    assert.equal(hrefs.length, 1, `${marker}: one href only`);
    assert.equal(literal(hrefs[0]), href, `${marker}: exact reviewed destination`);
    return hrefs[0];
  }));
}
function audit(file, source = read(file)) {
  ...
  const visit = node => {
    ...
    if (ts.isJsxAttribute(node) && /^(href|value|defaultValue|disabled|key|ref|type|checked|defaultChecked|required|accept|multiple|action|method)$/.test(node.name.getText(ast)) && !reviewedHrefs.has(node)) row.controls.push(print());
...
test('reviewed Publish navigation exception rejects a changed, missing, duplicate or misplaced link', () => {
  ...
  for (const [original, changed] of [
    ['href="/clips/new"', 'href="/clips/other"'],
    ['experience-release-path experience-release-post', 'unreviewed-post-path'],
    ['className="experience-release-paths"', 'className="unreviewed-release-paths"'],
    ['<Link href="/clips/new" className="experience-release-path experience-release-clip">', '<Link href="/clips/new" className="experience-release-path experience-release-clip" /><Link href="/clips/new" className="experience-release-path experience-release-clip">'],
  ]) {
    assert.ok(source.includes(original), original);
    assert.throws(() => audit('app/publish/page.tsx', source.replace(original, changed)), undefined, changed);
  }
});
...
test('AI archive retains real counters and every track action with readable identity and wrapping controls', () => {
  ...
  for (const value of ['stats.total','stats.favorites','stats.totalDuration','filteredGenerations.map','filteredTracks.map','generation.tracks?.map']) assert.ok(source.includes(value));
